...
function ToolIcon({ tool }: { tool: Tool }) {
  return (
    <div className="flex flex-col items-center justify-center gap-3 w-28 shrink-0">
      <div className="neo-card rounded-lg w-20 h-20 flex items-center justify-center neo-card-hover">
        {tool.image ? (
          <img
            src={tool.image}
            alt={`${tool.name} logo`}
          ...
          <img
            src={`https://cdn.simpleicons.org/${tool.slug}/${tool.color ?? "000000"}`}
            alt={`${tool.name} logo`}
            className="tool-icon w-10 h-10"
            loading="lazy"
          />
        ) : (
          <div className="tool-icon w-10 h-10 rounded-lg text-[#FFD60A] drop-shadow-[1px_1px_0_#000] font-bold text-xl flex items-center justify-center bg-[#f5f5f5]">
            {tool.name
              .split(" ")
              .map((w) => w[0])
              .slice(0, 2)
      ...
      <span className="text-xs text-black/60 text-center leading-tight">
        {tool.name}
      </span>
    </div>
  );
      ...
      <div className="max-w-6xl mx-auto">
        <div className="reveal mb-10 flex items-end justify-between gap-6 flex-wrap">
          <div>
            ...
            <h2 className="mt-3 display-font text-3xl md:text-4xl font-semibold tracking-tight text-black">
              Tools I build <span className="underline decoration-4 decoration-[#FFD60A]">with</span>
            </h2>
          </div>
        ...
        {/* Marquee */}
        <div className="reveal marquee-wrap relative overflow-hidden neo-card rounded-lg p-6">
          <div
            aria-hidden
